"""
Scanner de Arquivos - Back-end
--------------------------------
Servidor HTTP puro (biblioteca padrão do Python, sem Flask) que:
- Serve o front-end (HTML/CSS/JS) da pasta ./frontend
- Expõe a rota POST /api/scan que recebe um caminho de pasta,
  escaneia recursivamente os arquivos de TEXTO encontrados,
  gera um resumo.txt dentro da própria pasta escaneada
  e retorna tudo em JSON pro front-end exibir.
"""
...
EXTENSOES_TEXTO = {
    ".txt", ".py", ".json", ".csv", ".md", ".html", ".htm", ".css", ".js",
    ".xml", ".yaml", ".yml", ".log", ".ini", ".cfg", ".conf", ".sql",
...
def montar_texto_resumo(pasta_pai: Path, resumo_txt: Path, arquivos: list) -> str:
    linhas = []
    ...
    for item in arquivos:
        linhas.append(f'Caminho do ({Path(item["caminho"]).name}):')
...
def escanear_pasta(caminho_pasta: str) -> dict:
    ...
    texto_resumo = montar_texto_resumo(pasta_pai, resumo_txt_path, arquivos)
...
class ScannerRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _servir_download(self, query_string: str):
        ...
        self.send_header("Content-Disposition", 'attachment; filename="resumo.txt"')
